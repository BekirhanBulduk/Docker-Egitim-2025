...
# aynı bridge network ağı üzerinden first ve second isimli 2 farklı container arası trafik iletişim konstrolu 
...
root//
    ...
    ifconfig        <ip adresini not alalım >172.17.0.2
...
docker container run -d --cpus="1.5" --name cpuscontainer redis

* veya       <<<<<<<<cpus = core sayısı ARALIĞI spesifik cpu seçimi>>>>>>>>>>>>>. aşağdıa ise şu cpu ları kullan diyerek core numarası belirtebiliriz.

docker container run -d --cpus="1.5" --cpuset-cpus="1,3" --name cpuscontainersps nginx
...
* docker container run -d --env MYVAR=myvalue --env MYVAR-E=myvalue2 --name myenvcont nginx
